 ...
 <%
                                		...
                                		while (rsGr.next()) {
                                			...
                                			if(month == currentMonth){
                                				if(c_plan.equalsIgnoreCase("basic")) count[0]++;
                                				else if(c_plan.equalsIgnoreCase("standard")) count[1]++;
                                				else if(c_plan.equalsIgnoreCase("profession")) count[2]++;
                                				else if(c_plan.equalsIgnoreCase("custom")) count[3]++;
                                			}
...
<script language="JavaScript">
		...
		/*, 
						    ...
						    //체크박스 전체선택
					 	    ...
					 	    $("#PRM_index_main").click(function(){
					   	        //클릭되었으면
					   	        if($("#PRM_index_main").prop("checked")){
					   	            //input태그의 name이 chk인 태그들을 찾아서 checked옵션을 true로 정의
					   	            $("input[name=PRM_index]").prop("checked",true);
					   	            //클릭이 안되있으면
					   	        }else{
					   	            //input태그의 name이 chk인 태그들을 찾아서 checked옵션을 false로 정의
					   	            $("input[name=PRM_index]").prop("checked",false);
					   	        }
					   	    })
										...
										<div>
											<FORM method="post" name="register" id="register"
												...
												&nbsp;<LABEL>계산서발행일</LABEL> <INPUT type="text"
													name="bill_date2" id="bill_date2" size="5"
													style="border: 2px solid #000000;">&nbsp;&nbsp; <LABEL>고객사명</LABEL>
												<INPUT type="text" name="company_name" id="company_name"
													size="12" style="border: 2px solid #000000;">&nbsp;&nbsp;


												<LABEL>요금제</LABEL> <SELECT NAME="rate_system"
													id="rate_system" onchange="convert_fee(this.form)"
...
6개월/1년 계약은 동일하게 선택</textarea>
										...
										<div >
											...
											<form method=post id="PRM_search_period"
												...
												<BUTTON type="submit" id="PRM_period_btn"
													name="PRM_period_btn" onclick="PRM_period_btn_clicked()"
													class="btn" style="border: 2px solid #737373;">검색</button>
											</form>
											<SCRIPT type="text/javascript">
												spacing(4);
											</SCRIPT>
											<form method="post" style="display: inline">
												<LABEL>고객사코드</LABEL> <INPUT type="text"
													name="PRM_customer_code_for_search"
													id="PRM_customer_code_for_search" size="12"
													style="border: 2px solid #737373;">&nbsp;&nbsp;
												<BUTTON type="submit" class="btn"
													onclick="PRM_search_btn_clicked()"
										...
										<div ><h3>내역</h3></div>
                                         ...
                                         //실적관리 !!!!!!  
                                  		...
                                  		String sqlRM = "select * from CUSTOMER where userCode=?";
		...
		<tr>
			...
			<td style="margin: auto; width: 100%">
				<div
								...
								<h2 style="text-align: right; color:red; margin-top:15px"> <%=price[currentMonth-1] %>원 </h2>
								
								</div>
								<div id="flip2" style="border: 1px solid #ccc; box-shadow: 3px 3px 2px #666; padding: 15px; margin-left:10px"> <h4 style="font-weight:bold">요금제별 건수 보기</h4> 
