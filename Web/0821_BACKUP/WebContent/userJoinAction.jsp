...
<%
	...
	*/
	...
	sql = " select count(*) from partner where userType=? and userRegion=?";
	PreparedStatement pstmt = conn.prepareStatement(sql);
	pstmt.setString(1, userType);
	pstmt.setString(2, userRegion);
	ResultSet Num = pstmt.executeQuery();

	if (Num.next()) {
		int codeNum = Integer.parseInt(Num.getString(1));
		if (codeNum >= 0 && codeNum < 999) {
			...
			codeNumber = codeType + alph[first] + alph[second] + alph[third];
		}
	} else {
	...
	if (dupComp == 0 && dupEmail == 0) {
		...
		sql = "insert into partner(userCode, userName, userEmail, userPassword, userCompany, userTel, userRegion,userType) VALUES(?,?,?,?,?,?,?,?)";
		pstmt = conn.prepareStatement(sql);
