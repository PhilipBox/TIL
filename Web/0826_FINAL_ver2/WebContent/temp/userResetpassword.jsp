...
<script>
	function emailCheck() {
		...
		if (isEmpty(userEmail)) {
			...
			$("button[name='resetBtn']").attr("disabled", true);
		} else if (!(regEmail.test(userEmail))) {
			$("input[name='email']").css("border-color", "red");
			$("button[name='resetBtn']").attr("disabled", true);
		} else {
			$("input[name='email']").css("border-color", "rgb(205,205,205)");
			$("button[name='resetBtn']").attr("disabled", false);
		}
	}
	...
	<div
		...
		<div class="row full-height ng-scope">
			...
			<div id="KFrame" style="display: block"
				...
				<div class="row oauth-form ng-scope company-domain">
					<div class="well well-lg col-md-6 col-md-offset-3 sexy-well">
						<h3 class="text-center ng-scope">비밀번호 재설정</h3>
						<form class="pad-t-10 ng-pristine ng-invalid ng-invalid-required"
							name="formEmail" method="post" action="userResetAction.jsp">
							<div class="">
								<div class="form-group">
									<label translate="" class="ng-scope">이메일</label> <input
										type="email" id="KorEmail" name="email"
										class="form-control input-lg ng-pristine ng-invalid ng-invalid-required ng-valid-email ng-valid-maxlength"
										placeholder="이메일을 입력하세요" required="required"
										onkeyup="emailCheck();">
								</div>
								<button class="btn btn-lg btn-primary btn-block ng-scope"
									id="KorResetBtn" name="resetBtn" value="resetBtn" type="submit"
									disabled="disabled">비밀번호 재설정</button>
							</div>
						</form>
						<hr>
						<div class="row">
			...
			<div id="JFrame" style="display: none"
				...
				<div class="row oauth-form ng-scope company-domain">
					<div class="well well-lg col-md-6 col-md-offset-3 sexy-well">
						<h3 class="text-center ng-scope" style="font-size: 29px">パスワードを再設定</h3>
						<form class="pad-t-10 ng-pristine ng-invalid ng-invalid-required"
							name="formEmail">
							<div class="">
								<div class="form-group">
									<label translate="" class="ng-scope">メールアドレス </label> <input
										type="email" id="JapEmail" name="email"
										class="form-control input-lg ng-pristine ng-invalid ng-invalid-required ng-valid-email ng-valid-maxlength"
										placeholder="メールアドレスを入力してください。" required="required">
								</div>
								<button class="btn btn-lg btn-primary btn-block ng-scope"
									id="JapResetBtn" name="resetBtn" value="resetBtn" type="submit"
									disabled="disabled">パスワードを再設定</button>
							</div>
						</form>
						<hr>
						z
						<div class="row">
							<div class="col-xs-12 text-center">
								<a translate="" class="ng-scope" href="./userSignin.jsp">すでにアカウント登録済みですか？
								</a>
							</div>
						</div>
					</div>
