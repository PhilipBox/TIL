...
<%
	...
	try {
		...
		String sql = "SELECT title,memo, notice FROM board WHERE NUM=" + idx;
...
<script>
	// 자바 스크립트 시작
	function locateBack() {
		if (window.confirm("이 페이지를 벗어나면 수정된 내용은 저장되지 않습니다.")) {
	...
	function modifyOk(id) {
		//alert("notice Check" + $("#noticeCheck").val());
		$.ajax({
			type : "post",
			url : "./boardModifyOK.jsp",
			data : {
				idx : id,
				title : $("#title").val(),
				memo : $("#memo").val(),
				notice : $("#noticeCheck").val()
			//0안내 1공지
			},
			success : moveSuccess,
			error : moveError
		...
		<div class="b-fullheight ng-isolate-scope b-content">
			...
			<div class="ng-scope" id="listDiv" style="display: block;">
				...
				<table class="type01" width="1200px">
					...
					<tr>
						<td colspan="1" style="text-align: center;">제목</td>
						<td colspan="4"><input id="title" size="115" maxlength="100"
							value="<%=title%>"
							style="font-size: 13; outline: none; padding: 10px; border: 0;"></td>
						<td colspan="1" align="center"><select
							style="width: 90px; height: 30px" id="noticeCheck">
								<%
									if (notice == 1) {
								%>
								<option class="ng ng-scope" value="1" selected="selected">공지</option>
								<option class="ng ng-scope" value="0">안내</option>
								<%
									} else {
								%>
								<option class="ng ng-scope" value="1">공지</option>
								<option class="ng ng-scope" value="0" selected="selected">안내</option>
								<%
									}
								%>>
						</select></td>
					...
					<tr>
						<td colspan="1"
							style="text-align: center; padding: 150px 60px 150px 60px;">내용</td>
						<td colspan="5"><textarea name="memo" id="memo"
								class="nse_content"><%=memo%></textarea> <script
